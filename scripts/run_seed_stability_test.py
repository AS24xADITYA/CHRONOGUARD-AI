"""Multi-Seed Stability and Reproducibility Benchmark for ChronoGuard.

Evaluates 5 independent random initializations (seeds 42, 123, 456, 789, 2024)
under the strict 3-way chronological split (65% Train, 10% Val, 25% Test).

For each seed:
1. Re-seeds weight initialization, PyTorch RNG, numpy, and DataLoader shuffling.
2. Trains the Standard Attention LSTM for 15 epochs.
3. Monitors Macro F1 strictly on the Validation slice and selects the peak epoch.
4. Evaluates the selected checkpoint exactly once on the untouched Test slice.
5. Evaluates the Logistic Regression baseline under the same seed.
6. Computes mean and standard deviation across all 5 runs.
"""
...
def main():
    print("=== ChronoGuard 5-Seed Stability & Reproducibility Benchmark ===")
    ...
    print(f"\nRunning 5 independent seeds: {seeds}")
    print("=" * 95)
    ...
    for run_idx, seed in enumerate(seeds, 1):
        ...
        print(f"  Baseline: Test Macro F1 = {bl_eval['macro_f1']*100:.2f}%, Recon F1 = {bl_eval['per_stage']['Reconnaissance']*100:.2f}%, LatMove = {bl_eval['per_stage']['Lateral Movement']*100:.2f}%")
        ...
        for ep in range(1, 16):
            ...
            val_trajectory.append((ep, val_f1, val_res["per_stage"]["Reconnaissance"]))
        ...
        test_eval["val_trajectory"] = val_trajectory
        lstm_results.append(test_eval)

        print(f"  LSTM: Peak Val Epoch = {best_val_ep} (Val F1 = {best_val_f1*100:.2f}%)")
        print(f"  LSTM: Test Macro F1 = {test_eval['macro_f1']*100:.2f}%, Recon F1 = {test_eval['per_stage']['Reconnaissance']*100:.2f}%, LatMove = {test_eval['per_stage']['Lateral Movement']*100:.2f}%, t+1 F1 = {test_eval['t1_f1']*100:.2f}%")

    # Output Full Multi-Seed Table
    print("\n\n" + "=" * 105)
    print("                      CHRONOGUARD 5-SEED STABILITY & REPRODUCIBILITY RESULTS")
    print("=" * 105)
    ...
    lstm_lats = [r["per_stage"]["Lateral Movement"] * 100 for r in lstm_results]
    lstm_epochs = [r["selected_epoch"] for r in lstm_results]

    print("\n--- Summary Statistics Across 5 Seeds ---")
    ...
    print(f"LSTM Lateral Movement F1:     Mean = {np.mean(lstm_lats):.2f}% ± {np.std(lstm_lats):.2f}% (All zero: {all(x == 0 for x in lstm_lats)})")

    # Save summary JSON
    summary_data = {
        "seeds_evaluated": seeds,
        "baseline": {
            "mean_macro_f1": round(float(np.mean(bl_f1s)), 2),
            "std_macro_f1": round(float(np.std(bl_f1s)), 2),
            "runs": [
                {"seed": r["seed"], "macro_f1": round(r["macro_f1"], 4)} for r in baseline_results
            ],
        },
        "lstm": {
            "mean_macro_f1": round(float(np.mean(lstm_f1s)), 2),
            "std_macro_f1": round(float(np.std(lstm_f1s)), 2),
            "mean_t1_f1": round(float(np.mean(lstm_t1s)), 2),
            "std_t1_f1": round(float(np.std(lstm_t1s)), 2),
            "mean_recon_f1": round(float(np.mean(lstm_recons)), 2),
            "std_recon_f1": round(float(np.std(lstm_recons)), 2),
            "selected_epochs": lstm_epochs,
            "runs": [
                {
                    "seed": r["seed"],
                    "selected_epoch": r["selected_epoch"],
                    "val_macro_f1": round(r["val_macro_f1"], 4),
                    "test_macro_f1": round(r["macro_f1"], 4),
                    "t1_f1": round(r["t1_f1"], 4),
                    "recon_f1": round(r["per_stage"]["Reconnaissance"], 4),
                    "cred_f1": round(r["per_stage"]["Credential Access"], 4),
                    "lat_f1": round(r["per_stage"]["Lateral Movement"], 4),
                }
                for r in lstm_results
            ],
        },
    ...
    with open("results/stability_benchmark.json", "w") as f:
        json.dump(summary_data, f, indent=2)
    print("\nSaved results/stability_benchmark.json")
